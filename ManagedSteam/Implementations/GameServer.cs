...
namespace ManagedSteam.Implementations
{
    class GameServer : SteamServiceGameServer, IGameServer
    {
        ...
        public AuthTicketHandle GetAuthSessionTicket(byte[] ticket, out uint ticketSize)
        {
            CheckIfUsable();
            ticketSize = 0;

            using (NativeBuffer ticketBuffer = new NativeBuffer(ticket))
            {
                uint result = NativeMethods.GameServer_GetAuthSessionTicket(ticketBuffer.UnmanagedMemory,
                    ticketBuffer.UnmanagedSize, ref ticketSize);
                ticketBuffer.ReadFromUnmanagedMemory((int)ticketSize);
                return new AuthTicketHandle(result);
            }
        }

        public GameServerGetAuthSessionTicketResult GetAuthSessionTicket(byte[] ticket)
        {
            GameServerGetAuthSessionTicketResult result = new GameServerGetAuthSessionTicketResult();
            result.Result = GetAuthSessionTicket(ticket, out result.TicketSize);
            return result;
        }

        public BeginAuthSessionResult BeginAuthSession(byte[] authTicket, SteamID steamID)
        {
            CheckIfUsable();
            using (NativeBuffer ticketBuffer = new NativeBuffer(authTicket))
            {
                ticketBuffer.WriteToUnmanagedMemory();
                int tempreturn = (NativeMethods.GameServer_BeginAuthSession(ticketBuffer.UnmanagedMemory, ticketBuffer.UnmanagedSize, steamID.AsUInt64));
                return (BeginAuthSessionResult)tempreturn;
            }
